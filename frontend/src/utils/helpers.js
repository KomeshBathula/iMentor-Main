...
// Helper to format topic names from internal IDs (e.g., machine_learning.core_idea.generalize)
export const formatTopicName = (topic, short = false) => {
  if (!topic) return '';

  // If short mode, just take the last part
  if (short && topic.includes('.')) {
    topic = topic.split('.').pop();
  }

  // Helper to capitalize words in a string with underscores
  ...
  if (short) {
    return formatSegment(topic);
  }

  // Full path with arrows
  return topic.split('.')
    .map(formatSegment)
    .join(' › ');
};