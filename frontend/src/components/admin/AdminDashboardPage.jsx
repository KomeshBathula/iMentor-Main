...
import SyllabusGraphManager from './SyllabusGraphManager.jsx';
...
import { UploadCloud, Trash2, Eye, LogOut, Loader2, AlertTriangle, CheckCircle, RefreshCw, Shield, Users, Lightbulb, HelpCircle, Cog, Database, BarChart2, Gamepad2, Network, Link2, Brain, MessageSquareDiff } from 'lucide-react';
...
function AdminDashboardPage() {
    ...
    const [isMultiModelModalOpen, setIsMultiModelModalOpen] = useState(false); // 2.1.3 Multi-Model Management
    const [isLearningProfilesModalOpen, setIsLearningProfilesModalOpen] = useState(false); // Student learning profiles
    const [isUserFeedbackModalOpen, setIsUserFeedbackModalOpen] = useState(false); // Product feedback
        ...
        <div className="h-screen flex flex-col bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark">
            <header className="flex-shrink-0 flex items-center justify-between p-4 sm:p-6 border-b border-border-light dark:border-border-dark">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <IconButton icon={Brain} onClick={() => setIsLearningProfilesModalOpen(true)} title="Student Learning Profiles" variant="ghost" size="md" className="text-text-muted-light dark:text-text-muted-dark hover:text-primary" />
                    ...
                    <IconButton icon={Link2} onClick={() => setIsMultiModelModalOpen(true)} title="Multi-Model Management" variant="ghost" size="md" className="text-text-muted-light dark:text-text-muted-dark hover:text-pink-400" />
                    <IconButton icon={MessageSquareDiff} onClick={() => setIsUserFeedbackModalOpen(true)} title="User Feedback" variant="ghost" size="md" className="text-text-muted-light dark:text-text-muted-dark hover:text-amber-400" />
            ...
            {/* NEW: Gamification Modal */}
            <Modal isOpen={isGamificationModalOpen} onClose={() => setIsGamificationModalOpen(false)} title="Gamification Dashboard" size="5xl">
                <GamificationDashboard />
            </Modal>
            <Modal isOpen={isLearningProfilesModalOpen} onClose={() => setIsLearningProfilesModalOpen(false)} title="Student Learning Profiles" size="5xl">
                <AdminLearningProfiles />
            </Modal>
            ...
            {/* 2.1.3 Multi-Model Management Modal */}
            <Modal isOpen={isMultiModelModalOpen} onClose={() => setIsMultiModelModalOpen(false)} title="Multi-Model Management — Course ↔ Adapter Mapping" size="5xl">
                <MultiModelManager />
            </Modal>
            {/* User Product Feedback Modal */}
            <Modal isOpen={isUserFeedbackModalOpen} onClose={() => setIsUserFeedbackModalOpen(false)} title="User Feedback — Bugs, Suggestions & Reports" size="5xl">
                <UserFeedbackManager />
            </Modal>
            <Modal
