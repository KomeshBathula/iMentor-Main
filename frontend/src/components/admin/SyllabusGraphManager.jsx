...
import { FlaskConical, Upload, Loader2, CheckCircle, AlertTriangle, FileSpreadsheet, Eye, Trash2 } from 'lucide-react';
...
function SyllabusGraphManager() {
    ...
    const handleDeleteSyllabus = async () => {
        ...
        if (!targetCourse) {
            toast.error('Enter a course name to delete its syllabus graph');
            return;
        }

        const confirmed = window.confirm(
            `Delete syllabus graph for "${targetCourse}"? This removes curriculum graph data and related progress/cache entries.`
        );
        if (!confirmed) return;

        setIsDeleting(true);
        const toastId = toast.loading(`Deleting syllabus graph for ${targetCourse}...`);

        try {
            const response = await adminApi.deleteCourseGraph(targetCourse);
            setResult({
                success: true,
                message: response?.message || `Deleted syllabus graph for ${targetCourse}`,
                modulesCreated: 0,
                topicsCreated: 0,
                subtopicsCreated: 0,
                totalNodes: 0,
            ...
            setVisualizedCourse('');
            toast.success(response?.message || `Deleted syllabus graph for ${targetCourse}`, { id: toastId });
        } catch (error) {
            const errorMessage = error.message || `Failed to delete syllabus graph for ${targetCourse}`;
            setResult({ success: false, message: errorMessage });
            toast.error(errorMessage, { id: toastId });
        } finally {
            setIsDeleting(false);
        ...
        <div className="card-base p-4">
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            ...
                            disabled={!courseName.trim() || isUploading || isDeleting}
                ...
                <Button
                    ...
                    disabled={!selectedFile || !courseName.trim() || isUploading || isDeleting}
