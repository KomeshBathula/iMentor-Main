...
function ChatHistoryModal({ isOpen, onClose, onSelectSession, filterMode = null }) {
    const [sessions, setSessions] = useState([]);
    const [selectedSessionId, setSelectedSessionId] = useState(null);
    const [selectedSessionData, setSelectedSessionData] = useState(null); // Full session data including tutorMode
    ...
    const fetchSessions = useCallback(async () => {
        ...
        try {
            ...
            // Apply filtering based on mode if requested
            if (filterMode === 'tutor') {
    ...
    const handleSessionSelectForPreview = async (sessionId) => {
        ...
        try {
            ...
            // Store full session data for tutorMode check
    ...
    const handleLoadSessionAndClose = () => {
        if (selectedSessionId && selectedSessionData) {
            // Pass sessionId, tutorMode info, and tutorModeType to parent
            onSelectSession(selectedSessionId, {
                isTutorMode: selectedSessionData.isTutorMode || false,
                tutorModeType: selectedSessionData.tutorModeType || null,
                courseName: selectedSessionData.courseName || null
            });
            onClose();
        } else {
    ...
    return (
        <>
            <Modal isOpen={isOpen} onClose={onClose} title={`${filterMode === 'tutor' ? 'Tutor' : filterMode === 'general' ? 'General' : ''} Chat History`} size="2xl">
                <div className="flex flex-col md:flex-row gap-4 max-h-[70vh] h-[70vh]">
                    <div className="w-full md:w-1/3 border-r border-border-light dark:border-border-dark pr-0 md:pr-2 overflow-y-auto custom-scrollbar">
                        ...
                        {!loadingSessions && !sessions.length && <p className="text-xs text-text-muted-light dark:text-text-muted-dark p-2">No {filterMode || ''} sessions found.</p>}
                        <ul className="space-y-1">
                            {sessions.map(session => {
                                let badge = null;
                                if (session.isTutorMode) {
                                    badge = <span className="px-1.5 py-0.5 text-[0.6rem] bg-purple-500 text-white rounded-full flex-shrink-0">Tutor</span>;
                                }

                                return (
                                    <li key={session.sessionId} onClick={() => handleSessionSelectForPreview(session.sessionId)}
                                        className={`p-2.5 rounded-md cursor-pointer text-xs transition-colors group relative hover:shadow-sm
                                                    ${selectedSessionId === session.sessionId
                                                ? 'bg-primary text-white'
                                                : 'bg-surface-light dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700'}`} >

                                        <div className="font-medium truncate flex items-center gap-2" title={session.preview}>
                                            {session.preview || `Session ${session.sessionId.substring(0, 8)}`}
                                            {badge}
                                        </div>
