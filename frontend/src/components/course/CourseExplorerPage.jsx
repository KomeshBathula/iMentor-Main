...
function SubtopicItem({ subtopic, topicName, isActive, onClick }) {
    return (
        <button
            onClick={onClick}
            className={`w-full text-left flex items-center gap-2 px-3 py-1.5 rounded-md text-[13px] transition-all duration-150 cursor-pointer
                ${isActive
                    ? 'bg-indigo-500/10 text-indigo-200 font-medium border-l-2 border-indigo-400'
                    : 'text-gray-400 hover:text-gray-200 hover:bg-white/[0.04] border-l-2 border-transparent'}`}
        >
            <Hash size={11} className={`flex-shrink-0 mt-0.5 ${isActive ? 'text-indigo-400' : 'text-gray-600'}`} />
...
function TopicItem({ topic, activeSubtopic, onSubtopicSelect, defaultOpen }) {
    ...
    return (
        <div>
            <button
                onClick={() => setOpen(o => !o)}
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-300 hover:text-white hover:bg-white/[0.04] transition-all duration-150 cursor-pointer group"
            >
                ...
                <span className="truncate text-left text-[13px] flex-1">{topic.name || topic.id}</span>
...
function ModuleItem({ module, activeSubtopic, onSubtopicSelect, defaultOpen }) {
    ...
    return (
        <div className={`rounded-xl overflow-hidden transition-all duration-200 ${hasActive ? 'border border-indigo-500/20' : 'border border-white/[0.06]'}`}>
            <button
                onClick={() => setOpen(o => !o)}
                className="w-full flex items-center gap-2.5 px-4 py-3 text-left transition-all duration-150 hover:bg-white/[0.03] cursor-pointer"
            >
                <div className={`p-1.5 rounded-lg flex-shrink-0 ${hasActive ? 'bg-indigo-500/25' : 'bg-indigo-500/12'}`}>
                    <Layers size={13} className={hasActive ? 'text-indigo-300' : 'text-indigo-400/60'} />
                </div>
                <span className="text-[13px] font-semibold text-gray-200 truncate flex-1 leading-snug">
                    {module.name || module.id}
                </span>
                <span className="text-[11px] text-gray-500 flex-shrink-0 mr-1">
            ...
            {open && (
                <div className="px-3 pb-3 space-y-0.5 border-t border-white/[0.06] pt-2 mt-0">
...
// ─── Left Sidebar (tree) ───────────────────────────────────────────────────────
function CourseSidebar({ courseName, displayName, structure, activeSubtopic, onSubtopicSelect, onBack, sidebarOpen, setSidebarOpen, onTakeQuiz }) {
    const modules = structure?.modules || [];

    const activeModule = activeSubtopic
        ? modules.find(m => m.topics?.some(t => t.subtopics?.some(s => s.id === activeSubtopic.id)))
        : null;
            ...
            <div className="px-4 py-4 border-b border-white/[0.06] flex-shrink-0 space-y-2">
                <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-gray-600 uppercase tracking-widest font-semibold">Course</p>
                        <h2 className="text-sm font-bold text-white truncate leading-tight mt-0.5">{displayName}</h2>
                    </div>
                    <button
                        className="md:hidden p-1.5 text-gray-500 hover:text-white rounded-lg hover:bg-white/8 transition-all flex-shrink-0"
                ...
                <div className="flex items-center gap-3 text-[11px] text-gray-500 pl-9">
                    <span className="flex items-center gap-1">
                        <Layers size={10} className="text-indigo-500/70" />
                        {modules.length} modules
                    </span>
                    <span className="flex items-center gap-1">
                        <BookOpen size={10} className="text-indigo-500/70" />
                        {modules.reduce((acc, m) => acc + (m.topics?.length || 0), 0)} topics
                    </span>
                </div>

                {activeSubtopic && (
                    <div className="flex items-center gap-1.5 text-[11px] text-indigo-400/80 pl-9">
                ...
                <button
                    onClick={() => {
                        if (activeModule && typeof onTakeQuiz === 'function') {
                            onTakeQuiz({ moduleId: activeModule.id, moduleName: activeModule.name });
                        }
                    }}
                    disabled={!activeModule}
                    className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-semibold transition-all duration-150 cursor-pointer ${
                        activeModule
                            ? 'bg-indigo-500/12 hover:bg-indigo-500/25 border border-indigo-500/25 hover:border-indigo-500/50 text-indigo-300'
...
function CourseGrid({ onSelectCourse }) {
    ...
    return (
        <div className="min-h-screen bg-[#080a0e] text-white flex flex-col relative overflow-y-auto">
            ...
            <div className="flex-1 px-6 pb-16 max-w-5xl mx-auto w-full">
                ...
                {error && !loading && (
                    <div className="flex flex-col items-center justify-center py-20 gap-3 text-red-400">
                        ...
                        <p className="text-sm">{error}</p>
                        <button
                            onClick={() => { setError(null); setLoading(true); api.getSubjects().then(d => setCourses(Array.isArray(d.subjects) ? d.subjects : [])).catch(e => setError(e.message)).finally(() => setLoading(false)); }}
                ...
                {!loading && !error && filtered.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        {filtered.map((course, i) => {
                            ...
                            const cname = course.name || course.code;
                            return (
                                <button
                                    key={course.code}
                                ...
                                >
                                    ...
                                    <h3 className="text-base font-bold text-white mb-1 leading-tight">{cname}</h3>
                                    <p className="text-[12px] text-gray-500 leading-snug flex-1">
                                        {course.code}
                                        {course.semester && <span className="ml-2 text-gray-600">{course.semester}</span>}
                                        {course.credits != null && <span className="ml-2 text-gray-600">{typeof course.credits === 'object' ? `${course.credits.low}${course.credits.high ? '-' + course.credits.high : ''}` : course.credits} cr</span>}
                                    </p>
...
export default function CourseExplorerPage() {
    ...
    const handleSelectCourse = useCallback((code, name) => {
        setSelectedCourse(code);
        setSelectedCourseName(name || code);
    ...
    const handleAskAI = useCallback(() => {
        const displayName = selectedCourseName || selectedCourse;
        const prompt = activeSubtopic
            ? `Explain "${activeSubtopic.name}" from ${displayName}`
            : `Teach me about ${displayName}`;
        setInitialPromptForNewSession(prompt);
        setTutorMode(true);
        setTutorModeType('structured');
        navigate('/tutor');
    }, [activeSubtopic, selectedCourse, selectedCourseName, setInitialPromptForNewSession, setTutorMode, setTutorModeType, navigate]);
    ...
    const sidebarContent = loadingStruct ? (
        ...
        >
            <div className="px-4 py-4 border-b border-white/[0.06] space-y-2">
                <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex-1">
                        <p className="text-[10px] text-gray-600 uppercase tracking-widest font-semibold">Course</p>
                        <div className="h-3 bg-white/10 rounded w-32 mt-1.5 animate-pulse" />
                    </div>
                </div>
                <div className="flex items-center gap-3 text-[11px] text-gray-500 pl-9">
                    <div className="h-2 bg-white/8 rounded w-16 animate-pulse" />
                    <div className="h-2 bg-white/8 rounded w-14 animate-pulse" />
                </div>
            </div>
            <StructureLoading />
        </div>
        ...
        >
            <div className="px-4 py-4 border-b border-white/[0.06]">
                <div className="flex items-center gap-2.5">
                    ...
                    <p className="text-sm font-bold text-white">{selectedCourseName || selectedCourse}</p>
                </div>
            </div>
            <div className="flex flex-col items-center justify-center flex-1 gap-3 text-red-400 px-6 text-center">
                ...
                <button onClick={() => handleSelectCourse(selectedCourse, selectedCourseName)} className="text-[11px] text-indigo-400 hover:underline flex items-center gap-1">
        ...
        <CourseSidebar
            ...
            displayName={selectedCourseName || selectedCourse}
            structure={structure}
            activeSubtopic={activeSubtopic}
    ...
    return (
        ...
        >
            ...
            <div className="hidden md:flex flex-shrink-0 h-full overflow-hidden" style={{ width: '320px' }}>
                {sidebarContent}
            </div>
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden min-w-0">
                ...
                >
                    ...
                    <div className="flex-1 flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-sm font-semibold text-white truncate">{selectedCourseName || selectedCourse}</span>
                        <span className="text-[11px] text-gray-600 font-mono">{selectedCourse}</span>
                        {activeSubtopic && (
                            <>
