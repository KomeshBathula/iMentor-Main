...
const SkillTreeCourseSelector = ({
  ...
  const [courseMode, setCourseMode] = useState('database'); // database | other
  const [courseValue, setCourseValue] = useState('');
  const [manualCourseValue, setManualCourseValue] = useState('');

  const [isValidating, setIsValidating] = useState(false);
  const [validationStatus, setValidationStatus] = useState(null); // { status, canonical, suggestions }
  ...
  const handleChooseSuggestion = (item) => {
    ...
    setShowSuggestions(false);
    // Auto-treat a picked suggestion as validated so Continue is immediately enabled.
  ...
  const handleUploadCsv = async () => {
    ...
    try {
      console.log('[FRONTEND CSV] Upload button clicked');
      console.log('[FRONTEND CSV] csvFileName=', csvFileName);
      console.log('[FRONTEND CSV] csvText length=', csvText?.length || 0);
      ...
      console.log('[FRONTEND CSV] response=', res.data);
      ...
      const errorMsg = e.response?.data?.message || e.response?.data?.reason || 'CSV upload/matching failed';
  ...
  const onFilePicked = async (file) => {
    ...
    // Reset matching result so user can see fresh match
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            onClick={() => {
              if (courseMode === 'other') {
                // Other Course: continue directly with the typed name, no validation needed.
                const name = (manualCourseValue || '').trim();
                if (!name) return;
                onValidatedCourse?.({ canonical: name, source: 'other' });
                return;
              }
              // Database mode: continue with canonical from validation, or the selected course name.
            ...
            disabled={
              ...
              (courseMode === 'other'
                ...
                // Database mode: enabled when something is typed/selected (validation optional)
