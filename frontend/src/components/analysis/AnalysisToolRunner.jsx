            ...
            if (response) {
                if (response.content && !response.content.startsWith("Error:") && !response.content.startsWith("Notice:")) {
                    ...
                    if (response.content.startsWith("Error:")) {
                         toast.error(`Error in ${title}: ${response.content.substring(0, 100)}...`, { id: toastId });
                    }
                } else {
                    setAnalysisContent(`No content was returned for ${title}.`);
                    setError(`No content returned for ${title}.`);
                    toast.warn(`No content generated for ${title}.`, { id: toastId });
                }
    ...
    return (
        <div className="card-base p-3">
            <div className="flex items-center justify-between">
                ...
                {isSectionOpen && (
                    <Animate key="tool-section-content" animation="fade-in" duration="0.25s" className="mt-2 pt-2 border-t border-border-light dark:border-border-dark overflow-hidden">
                        ...
                        {!isLoading && (analysisContent || aiReasoning) && isDropdownOpen && (
                            <Animate key="analysis-dropdown" animation="slide-up" duration="0.2s" className="mt-2 space-y-2">
