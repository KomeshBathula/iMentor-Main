...
async function getSummaryAndGaps(transcript, existingSummary, llmProvider, ollamaModel, userApiKey, userOllamaUrl) {
  ...
  try {
    ...
    if (SGLANG_ENABLED) {
      // [NIGHT_SHIFT] Full analysis runs at 2AM via nightly cron with the 32B model.
      // Live path: use SGLang chat endpoint for quick session summary — Ollama is not running.
      const sglangService = require('./sglangService');
      responseText = await sglangService.generateContentWithHistory(
        [], userPrompt, SUMMARY_GAPS_PROMPT,
        { endpoint: 'chat', maxOutputTokens: 2048, temperature: 0.2 }
      );
    } else {
      ...
      const llmService = activeProvider === 'ollama' ? ollamaService : (activeProvider === 'groq' ? groqService : geminiService);

      const llmOptions = {
        apiKey: providerResolution.apiKey,
        ollamaUrl: providerResolution.workingOllamaUrl || userOllamaUrl,
        model: (activeProvider === 'groq' ? 'llama-3.1-8b-instant' : ollamaModel),
        temperature: 0.2
      };
      responseText = await llmService.generateContentWithHistory([], userPrompt, SUMMARY_GAPS_PROMPT, llmOptions);
    }
    ...
    log.success('AI', `Session analysis complete (${keyTopics.length} topics identified)`);
    return { summary: finalSummary, knowledgeGaps, keyTopics };

  } catch (error) {
    log.error('AI', `Session analysis failed: ${error.message}`);
...
async function generateRecommendations(knowledgeGaps, llmProvider, ollamaModel, userApiKey, userOllamaUrl) {
  ...
  try {
    ...
    if (SGLANG_ENABLED) {
      const sglangService = require('./sglangService');
      responseText = await sglangService.generateContentWithHistory(
        [], userPrompt, RECOMMENDATIONS_PROMPT,
        { endpoint: 'chat', maxOutputTokens: 1024, temperature: 0.5 }
      );
    } else {
      const providerResolution = await resolveProviderByPreference({
        preferredProvider: llmProvider,
      ...
      responseText = await llmService.generateContentWithHistory([], userPrompt, RECOMMENDATIONS_PROMPT, llmOptions);
    }
    ...
    log.success('AI', `Generated ${recommendations.length} recommendations`);
    return recommendations;
  } catch (error) {
    log.error('AI', `Recommendation generation failed: ${error.message}`);
    return [];
  }
}
