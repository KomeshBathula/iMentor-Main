...
async function generateContentWithHistory(
    ...
    try {
        ...
        let completion;
        let attempts = 0;
        const maxAttempts = 3; // [Optimization] Capped from 12 — prevent 60s+ event loop blocking
        while (attempts < maxAttempts) {
            try {
                ...
                if (isRateLimit && attempts < maxAttempts) {
                    // [Optimization] Exponential backoff: 2s, 4s, 8s — max ~15s total instead of 60s+
                    const baseWaitMs = Math.min(2000 * Math.pow(2, attempts - 1), 8000);
                    let waitMs = baseWaitMs;
                    const match = errMsg.match(/try again in (\d+(\.\d+)?)s/i);
                    if (match) {
                        waitMs = Math.min(Math.ceil(parseFloat(match[1]) * 1000) + 500, 10000);
                    }
