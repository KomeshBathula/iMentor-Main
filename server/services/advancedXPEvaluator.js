...
- Final XP: Round to integer, min 1, max 20`;
        ...
        // Use LLM to evaluate - standardized with intelligent routing
        ...
        const llmOptions = {
            ...
            apiKey: chosenModel.provider === 'gemini' ? process.env.GEMINI_API_KEY : (chosenModel.provider === 'groq' ? process.env.GROQ_API_KEY : null),
        ...
        let evaluationText;
        try {
            evaluationText = await llmService.generateContentWithHistory(
                [],
                prompt,
                "You are an educational evaluator. Respond with valid JSON only.",
                llmOptions
            );
        } catch (llmError) {
            log.error('AI', `${chosenModel.provider} evaluation failed: ${llmError.message}`);
        ...
        if (!jsonMatch) {
            log.warn('AI', 'Failed to parse evaluation response');
            return getFallbackEvaluation(userMessage, userLevel);
        }
        ...
        log.info('SYSTEM', `Quality Check: ${evaluation.xpAward} XP (${bloomsLevel})`);

        return evaluation;

    } catch (error) {
        log.error('SYSTEM', 'Error in XP evaluation', error);
...
function getFallbackEvaluation(message, userLevel) {
    ...
    // log.info('SYSTEM', 'Using fallback evaluation');
    ...
    // log.info('SYSTEM', `Fallback result: ${evaluation.xpAward} XP (${bloomsLevel})`);
...
async function getUserProgressionMultiplier(userId) {
    try {
        ...
        log.error('SYSTEM', 'Error calculating progression', error);
        return 1.0;
    }
}
