...
def delete_course_graph(course_name: str) -> bool:
    ...
    try:
        ...
        with driver.session(database=config.NEO4J_DATABASE) as session:
            # Delete :Concept nodes (syllabus-style)
            ...
            concept_count = concept_record['deleted_count'] if concept_record else 0

            # Also delete :Module/:Topic/:Subtopic nodes so no orphans remain
            # (mirrors what curriculum_graph_handler.delete_course_curriculum does)
            curriculum_result = session.run(
                """
                MATCH (n)
                WHERE (n:Module OR n:Topic OR n:Subtopic)
                  AND toLower(n.course) = toLower($course)
                DETACH DELETE n
                RETURN count(n) AS deleted_count
                """,
                course=course_name
            )
            curriculum_record = curriculum_result.single()
            curriculum_count = curriculum_record['deleted_count'] if curriculum_record else 0

            deleted_count = concept_count + curriculum_count

        logger.info(f"Deleted {deleted_count} nodes (concepts + curriculum) for course '{course_name}'")
        return True
