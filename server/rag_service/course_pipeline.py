...
try:
    ...
    logger.warning(f"Failed to import graph_rag: {e}")
    _graph_rag = None

try:
    import subtopic_notes_generator as _stn_gen
except ImportError as e:
    logger.warning(f"Failed to import subtopic_notes_generator: {e}")
    _stn_gen = None
...
def ingest_course(
    ...
    try:
        ...
        for filename in os.listdir(materials_folder):
            ...
            try:
                ...
                if processed_chunks:
                    ...
                    num_added = current_app.vector_service.add_processed_chunks(processed_chunks)
                    doc_result["chunks_added"] = num_added
                    qdrant_stats["total_chunks_added"] += num_added
                else:
                    doc_result["chunks_added"] = 0

                # Phase 2b: Extract entity-relation triples into Neo4j (background thread)
                if _graph_rag and raw_text:
                    import threading, asyncio

                    def _run_entity_extraction(text, doc_name, uid):
                        try:
                            asyncio.run(_graph_rag.extract_and_store_graph(text, doc_name, uid))
                            logger.info(f"[Course Pipeline] Entity extraction done: {doc_name}")
                        except Exception as _e:
                            logger.warning(f"[Course Pipeline] Entity extraction failed for {doc_name}: {_e}")

                    threading.Thread(
                        target=_run_entity_extraction,
                        args=(raw_text, filename, user_id),
                        daemon=True,
                        name=f"entity_extract:{filename}",
                    ).start()
        ...
        # Invalidate stale STN cache — new material supersedes cached teaching notes
        if _stn_gen and qdrant_stats.get("total_chunks_added", 0) > 0:
            try:
                _stn_gen.invalidate_course_stn(course_name)
                logger.info(f"[Course Pipeline] STN cache invalidated for '{course_name}'")
            except Exception as _e:
                logger.warning(f"[Course Pipeline] STN invalidation failed: {_e}")
...
def get_topic_context(course: str, topic_id: str) -> Dict[str, Any]:
    ...
    if result.get("topic"):
        try:
            ...
            if topic_name:
                # Obtain the active vector service singleton from Flask
                from flask import current_app
                vector_svc = current_app.vector_service
                
                # Construct filter for user_id="admin"
                from qdrant_client import models as qdrant_models
                qdrant_filters = qdrant_models.Filter(
                    must=[
                        qdrant_models.FieldCondition(
                            key="user_id",
                            match=qdrant_models.MatchValue(value="admin")
                        )
                    ]
                )

                # Search Qdrant for chunks related to this topic
                context_docs, snippet, docs_map = vector_svc.search_documents(
                    query=topic_name,
                    k=5,
