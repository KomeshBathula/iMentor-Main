...
class SyllabusQdrantLinker:
    ...
    def enrich_metadata_with_syllabus(
        ...
        if ctx:
            ...
            # Scalar fields for each subtopic — enables Qdrant keyword filter per subtopic
            for i, st in enumerate(ctx.subtopics[:5]):  # cap at 5 to avoid bloat
                metadata[f'syllabus_subtopic_{i}'] = st.lower().strip()
