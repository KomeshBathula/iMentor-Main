...
def clean_and_normalize_text_content(text: str, file_base_name_for_log: str ="") -> str:
    ...
    nlp_model = config.get_spacy_model()
    if not nlp_model:
    ...
    try:
        ...
        doc = nlp_model(text_lower, disable=['parser', 'ner']) # Disable unused pipes
        lemmatized_tokens = [
...
def extract_document_metadata_info(
    ...
    try:
        ...
        # virtual:// paths (text_content_override) have no OS entry — expected, not a real warning
        log_fn = logger.debug if str(file_path).startswith("virtual://") else logger.warning
        log_fn(f"Metadata: OS metadata error for {original_file_name}: {e_os_meta}")
    ...
    nlp_model = config.get_spacy_model()
    if processed_text and nlp_model:
        ...
        try:
            ...
            spacy_doc = nlp_model(text_for_ner) # NER pipe should be enabled by default
...
def chunk_document_into_segments(
    ...
    # --- CHAPTER/SECTION BASED CHUNKING STRATEGY ---
    # 1. Attempt to identify logical sections first
    # Patterns: Markdown headers (#, ##, ###) or "Chapter X", "Section Y" at start of lines
    header_pattern = r'(?m)^(?:\#{1,3}\s+|Chapter\s+\d+|Section\s+\d+).*$'
    
    # Store tuples of (section_title, section_content)
    # If no headers found, treat entire text as one section "General"
    sections: List[Tuple[str, str]] = []
    
    try:
        matches = list(re.finditer(header_pattern, text_to_chunk))
        if matches:
            logger.info(f"Chunking: Found {len(matches)} potential chapter/header boundaries in {original_doc_name_for_log}.")
            last_pos = 0
            for i, match in enumerate(matches):
                # Content before this header (belongs to previous section or preamble)
                pre_content = text_to_chunk[last_pos:match.start()].strip()
                if pre_content:
                    # If it's the very first block, call it 'Preamble' or 'Introduction', otherwise belongs to prev header
                    title = "Introduction/Preamble" if i == 0 else matches[i-1].group().strip()
                    # Clean title of markdown syntax
                    title = re.sub(r'^#+\s*', '', title).strip()
                    sections.append((title, pre_content))
                
                last_pos = match.start()
            
            # Catch the final section
            final_content = text_to_chunk[last_pos:].strip()
            if final_content:
                last_header = matches[-1].group().strip()
                title = re.sub(r'^#+\s*', '', last_header).strip()
                # The content includes the header itself, let's keep it that way for context
                sections.append((title, final_content))
        else:
             sections.append(("General Content", text_to_chunk))

    except Exception as e_regex:
        logger.warning(f"Chunking: Regex splitting failed for {original_doc_name_for_log}: {e_regex}. Falling back to flat split.")
        sections = [("General Content", text_to_chunk)]

    # 2. Process each section with RecursiveCharacterTextSplitter
    ...
    for section_title, section_text in sections:
        try:
            raw_text_segments = text_splitter.split_text(section_text)
            
            for segment_content in raw_text_segments:
                if not segment_content.strip(): continue
                ...
                chunk_specific_metadata['chunk_reference_name'] = f"{base_file_name_for_ref}_chunk_{global_chunk_index:04d}"
                chunk_specific_metadata['chunk_index'] = global_chunk_index
                chunk_specific_metadata['chunk_char_count'] = len(segment_content)
                
                # --- NEW METADATA FOR CHAPTER AWARENESS ---
                chunk_specific_metadata['section_context'] = section_title
                
                # --- SYLLABUS CONTEXT ENRICHMENT ---
                # If syllabus context was provided in document_level_metadata, propagate to chunks
                # This enables RAG queries to filter/sort by curriculum structure
                syllabus_fields = [
                    'syllabus_module', 'syllabus_module_order', 'syllabus_topic',
                    'syllabus_lecture_number', 'syllabus_subtopics', 'syllabus_chapter',
                    'syllabus_course', 'syllabus_context'
                ]
                for field in syllabus_fields:
                    if document_level_metadata.get(field) is not None:
                        chunk_specific_metadata[field] = document_level_metadata[field]
                
                # Also inject it into the text content for the model to see explicitly? 
                # Ideally, metadata is enough for filtering, but appending to text helps context.
                # Let's keep text clean but ensure vector DB searches verify metadata.
            ...
            logger.error(f"Chunking: Error splitting section '{section_title}' for {original_doc_name_for_log}: {e_split}")
            continue
    
    logger.info(f"Chunking: Split '{original_doc_name_for_log}' into {len(output_chunks)} non-empty chunks across {len(sections)} sections.")
...
def generate_segment_embeddings(document_chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    try:
        embeddings_np_array = embed_model.encode(texts_to_embed, show_progress_bar=True) # Set to True for long lists
...
def process_document_for_qdrant(
    ...
    try:
        ...
        # [DAY_SHIFT] KG is admin-only during live hours — non-admin uploads skip KG to save VRAM.
        # Nightly offline_ingest.py picks up deferred KG work at 2AM.
        if user_id and str(user_id).lower() != "admin":
            chunks_for_kg_worker = []
            logger.info(f"ai_core: Non-admin user_id={user_id} — KG deferred to nightly job")
        else:
