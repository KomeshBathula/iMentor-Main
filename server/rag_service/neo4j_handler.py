...
def decommission_course_curriculum(course_name: str):
    """
    Delete all curriculum nodes (Module, Topic, Subtopic) for a course.
    KnowledgeNodes and their relationships remain as they are general knowledge.
    """
    query = """
    MATCH (n)
    WHERE (n:Module OR n:Topic OR n:Subtopic) AND toLower(n.course) = toLower($courseName)
    DETACH DELETE n
    """
    try:
        from neo4j_handler import _execute_write_tx
        _execute_write_tx(lambda tx: tx.run(query, courseName=course_name))
        logger.info(f"Neo4j: Decommissioned curriculum for course '{course_name}'")
        return True
    except Exception as e:
        logger.error(f"Neo4j: Failed to decommission course '{course_name}': {e}")
        return False
