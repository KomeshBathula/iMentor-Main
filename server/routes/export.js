...
router.post('/podcast', async (req, res) => {
    ...
    try {
        ...
        activeProvider = providerResolution.chosenProvider;
        ...
        if (userSource?.textContent) {
            ...
            apiKeyForRequest = providerResolution.apiKey;
        } else {
        ...
        // If the provider REQUIRES a key (Gemini/Groq) but we don't have one, fail.
        if (['gemini', 'groq'].includes(activeProvider) && !apiKeyForRequest) {
            return res.status(400).json({ message: `An API Key for ${activeProvider} is required for podcast generation but is not configured.` });
        }
        ...
        // log.info('SYSTEM', "Forwarding podcast request to Python service");
        ...
        log.error('SYSTEM', `Podcast generation error: ${errorMsg}`);
