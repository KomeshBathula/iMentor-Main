...
const { resolveProviderByPreference } = require('../services/providerPriorityService');
const {
    validateCodeExecution,
    validateCodeAnalysis,
    validateTestCaseGen,
    validateExplainError,
...
async function getApiKeyForRequest(userId) {
  try {
    ...
    const decryptedUserKey = (user && user.encryptedApiKey) ? decrypt(user.encryptedApiKey) : null;
    const providerResolution = await resolveProviderByPreference({
      preferredProvider: user?.preferredLlmProvider || 'local_llm',
      userApiKey: decryptedUserKey,
      userOllamaUrl: null,
      skipOllamaHealthCheck: true,
    });

    if (providerResolution.apiKey) return providerResolution.apiKey;

  } catch (e) {
    log.error('SYSTEM', `Failed to get API key for ${userId}: ${e.message}`);
  }
...
router.post("/execute", validateCodeExecution, async (req, res) => {
...
router.post("/analyze-code", validateCodeAnalysis, async (req, res) => {
...
router.post("/generate-test-cases", validateTestCaseGen, async (req, res) => {
  const { language, code } = req.body;
  if (!code || !language) {
    return res.status(400).json({ message: "Code and language are required." });
  }
...
router.post("/explain-error", validateExplainError, async (req, res) => {
...
router.post("/generate-quiz", quizUpload.single("file"), async (req, res) => {
  ...
  try {
    ...
    // log.info('SYSTEM', "Forwarding quiz generation request to Python");
    ...
    log.error('SYSTEM', `Quiz generation error: ${errorMsg}`);
    res.status(error.response?.status || 500).json({ message: errorMsg });
  } finally {
    await fs.promises
      .unlink(file.path)
      .catch((err) =>
        log.error('SYSTEM', `Failed to delete temp quiz file: ${err.message}`)
      );
  }
});

router.post('/analyze-integrity/submit', validateIntegritySubmit, async (req, res) => {
  const { text } = req.body;
  ...
  try {
    ...
    const errorMsg = error.response?.data?.error || error.message;
    log.error('SYSTEM', `Integrity check submission error: ${errorMsg}`);
    res.status(error.response?.status || 500).json({ message: errorMsg });
  }
});
...
router.get('/analyze-integrity/report/:reportId', async (req, res) => {
  ...
  if (report.plagiarism && report.plagiarism.status === 'pending') {
    ...
    try {
      ...
      log.error('SYSTEM', `Integrity check poll error: ${error.message}`);
    }
  }
