...
2.3 Another algorithm for maximizing (cid:96)(θ) . . . . . . . . . . . . . 24
...
6.2 Notation (option reading) . . . . . . . . . . . . . . . . . . . . 62
6.3 Functional and geometric margins (option reading) . . . . . . 62
6.4 The optimal margin classifier (option reading) . . . . . . . . . 64
6.5 Lagrange duality (optional reading) . . . . . . . . . . . . . . . 66
6.6 Optimal margin classifiers: the dual form (option reading) . . 69
6.7 Regularization and the non-separable case (optional reading) . 73
6.8 The SMO algorithm (optional reading) . . . . . . . . . . . . . 74
6.8.1 Coordinate ascent . . . . . . . . . . . . . . . . . . . . . 75
6.8.2 SMO . . . . . . . . . . . . . . . . . . . . . . . . . . . . 76
II Deep learning 80
7 Deep learning 81
7.1 Supervised learning with non-linear models . . . . . . . . . . . 81
7.2 Neural networks . . . . . . . . . . . . . . . . . . . . . . . . . . 83
7.3 Backpropagation . . . . . . . . . . . . . . . . . . . . . . . . . 92
7.3.1 Preliminary: chain rule . . . . . . . . . . . . . . . . . . 93
7.3.2 One-neuron neural networks . . . . . . . . . . . . . . . 93
7.3.3 Two-layer neural networks: a low-level unpacked com-
putation . . . . . . . . . . . . . . . . . . . . . . . . . . 94
7.3.4 Two-layer neural network with vector notation . . . . . 97
7.3.5 Multi-layer neural networks . . . . . . . . . . . . . . . 99
7.4 Vectorization over training examples . . . . . . . . . . . . . . 99
III Generalization and regularization 102
8 Generalization 103
8.1 Bias-variance tradeoff . . . . . . . . . . . . . . . . . . . . . . . 105
8.1.1 A mathematical decomposition (for regression) . . . . . 110
8.2 The double descent phenomenon . . . . . . . . . . . . . . . . . 111

CS229 Spring 2022 3
8.3 Sample complexity bounds (optional readings) . . . . . . . . . 116
8.3.1 Preliminaries . . . . . . . . . . . . . . . . . . . . . . . 116
8.3.2 The case of finite H . . . . . . . . . . . . . . . . . . . . 118
8.3.3 The case of infinite H . . . . . . . . . . . . . . . . . . 121
9 Regularization and model selection 125
9.1 Regularization . . . . . . . . . . . . . . . . . . . . . . . . . . . 125
9.2 Implicit regularization effect . . . . . . . . . . . . . . . . . . . 127
9.3 Model selection via cross validation . . . . . . . . . . . . . . . 129
9.4 Bayesian statistics and regularization . . . . . . . . . . . . . . 132
IV Unsupervised learning 134
10 Clustering and the k-means algorithm 135
11 EM algorithms 138
11.1 EM for mixture of Gaussians . . . . . . . . . . . . . . . . . . . 138
11.2 Jensen’s inequality . . . . . . . . . . . . . . . . . . . . . . . . 141
...
reading) . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . 150
12 Principal components analysis 155
13 Independent components analysis 161
13.1 ICA ambiguities . . . . . . . . . . . . . . . . . . . . . . . . . . 162
13.2 Densities and linear transformations . . . . . . . . . . . . . . . 163
13.3 ICA algorithm . . . . . . . . . . . . . . . . . . . . . . . . . . . 164
14 Self-supervised learning and foundation models 167
...
14.3.1 Zero-shot learning and in-context learning . . . . . . . 173
...
16 LQR, DDP and LQG 193
16.1 Finite-horizon MDPs . . . . . . . . . . . . . . . . . . . . . . . 193
16.2 Linear Quadratic Regulation (LQR) . . . . . . . . . . . . . . . 197
16.3 From non-linear dynamics to LQR . . . . . . . . . . . . . . . 200
16.3.1 Linearization of dynamics . . . . . . . . . . . . . . . . 201
16.3.2 Differential Dynamic Programming (DDP) . . . . . . . 201
16.4 Linear Quadratic Gaussian (LQG) . . . . . . . . . . . . . . . . 203
17 Policy Gradient (REINFORCE) 207
...
n
(cid:88)
= y(i)logh(x(i))+(1−y(i))log(1−h(x(i)))
i=1
...
2.3 Another algorithm for maximizing (cid:96)(θ)
...
k parameters φ ,...,φ specifying the probability of each of the outcomes.
1 k
However, theseparameterswouldberedundant, ormoreformally, theywould
not be independent (since knowing any k−1 of the φ ’s uniquely determines
i
the last one, as they must satisfy
(cid:80)k
φ = 1). So, we will instead pa-
i=1 i
rameterize the multinomial with only k −1 parameters, φ ,...,φ , where
1 k−1
φ = p(y = i;φ), and p(y = k;φ) = 1−
(cid:80)k−1φ
. For notational convenience,
i i=1 i
we will also let φ = 1 −
(cid:80)k−1φ
, but we should keep in mind that this is
k i=1 i
not a parameter, and that it is fully specified by φ ,...,φ .
1 k−1
...
 0   1   0   0   0 
         
T(1)= 0 , T(2)= 0 , T(3)= 1 , ··· ,T(k−1)= 0 , T(k)= 0 ,
 .   .   .   .   . 
 .   .   .   .   . 
 .   .   .   .   . 
0 0 0 1 0
...
(T(y)) to denote the i-th element of the vector T(y).
i
...
(T(y)) = 1{y = i}. (Before you continue reading, please make sure you un-
i
derstandwhythisistrue!) Further, wehavethatE[(T(y)) ] = P(y = i) = φ .
i i
We are now ready to show that the multinomial is a member of the

32
exponential family. We have:
p(y;φ) = φ1{y=1}φ1{y=2}···φ1{y=k}
1 2 k
= φ1{y=1}φ1{y=2}···φ 1−(cid:80) i k = − 1 11{y=i}
1 2 k
= φ(T(y))1φ(T(y))2···φ 1−(cid:80)k i= − 1 1(T(y))i
1 2 k
...
φ
k
For convenience, we have also defined η = log(φ /φ ) = 0. To invert the
k k k
...
k i
i=1 i=1
This implies that φ = 1/ (cid:80)k eηi, which can be substituted back into Equa-
k i=1
...
To complete our model, we use Assumption 3, given earlier, that the η ’s
i
are linearly related to the x’s. So, have η = θTx (for i = 1,...,k − 1),
i i
where θ ,...,θ ∈ Rd+1 are the parameters of our model. For notational
1 k−1
convenience, we can also define θ = 0, so that η = θTx = 0, as given
k k k
...
p(y = i|x;θ), for every value of i = 1,...,k. (Even though h (x) as defined
θ
...
θ of this model, we would begin by writing down the log-likelihood
i
n
(cid:88)
(cid:96)(θ) = logp(y(i)|x(i);θ)
i=1
(cid:88) n (cid:89) k
(cid:32)
eθ l Tx(i)
...
the parameters by maximizing (cid:96)(θ) in terms of θ, using a method such as
...
θ
caseswhenh (x) = θ(cid:62)x(inlinearregressionorlogisticregression)orh (x) =
θ θ
θ(cid:62)φ(x) (where φ(x) is the feature map). A commonality of these two models
...
to think h (x) as an abstract non-linear model.1
θ
Suppose{(x(i),y(i))}n arethetrainingexamples. Forsimplicity, westart
i=1
with the case where y(i) ∈ R and h (x) ∈ R.
θ
...
h (x) = max(wx+b,0), where θ = (w,b) ∈ R2 (7.6)
θ
...
h (x) = ReLU(w(cid:62)x+b), where w ∈ Rd, b ∈ R, and θ = (w,b) (7.7)
θ
...
1 1 4
a = ReLU(w(cid:62)x+b ), where w ∈ R4 and b ∈ R (7.9)
1 1 1 1 1
a = ReLU(w(cid:62)x+b ), where w ∈ R4 and b ∈ R
2 2 2 2 2
a = ReLU(w(cid:62)x+b ), where w ∈ R4 and b ∈ R
3 3 3 3 3
We still define h (x) using equation (7.8) with a ,a ,a being defined
θ 1 2 3
...
edge from node x to node a indicates that a depends on x . The edge from
i j j i
x toa isassociatedwiththeweight(w[1]) whichdenotesthei-thcoordinate
i j j i
of the vector w[1]. The activation a can be computed by taking the ReLUof
j j
the weighted sum of x ’s with the weights being the weights associated with
i
the incoming edges, that is, a = ReLU( (cid:80)d (w[1]) x ).
j i=1 j i i
visualized in the dependency graph in Figure 7.3 because all the intermediate
variables a ’s depend on all the inputs x ’s.
i i
...
∀j ∈ [1,...,m], z = w[1](cid:62) x+b[1] where w[1] ∈ Rd,b[1] ∈ R (7.10)
j j j j j
a = ReLU(z ),
j j
a = [a ,...,a ](cid:62) ∈ Rm
1 m
h (x) = w[2](cid:62) a+b[2] where w[2] ∈ Rm,b[2] ∈ R, (7.11)
θ
...
[z ,...,z ](cid:62) ∈ Rm as
1 m
 z 1   — w 1 [1](cid:62) —   x 1   b[ 1 1] 
    . . . . . .     =     — w 2 [ . . . 1](cid:62) —         x . . . 2     +     b[ 2 . . . 1]     (7.13)
 
z m — w[1](cid:62) — x d b[ m 1]
m
...
θ
a[k] = ReLU(W[k]a[k−1] +b[k]),∀k = 1,...,r−1 (7.18)
...
h (x) = W[2]a[1] (7.21)
θ
= W[2]σ(z[1]) by definition (7.22)
= W[2]z[1] since σ(z) = z (7.23)
= W[2]W[1]x from Equation (7.13) (7.24)
= W ˜ x where W ˜ = W[2]W[1] (7.25)
...
Theorem 7.3.1: [backpropagation or auto-differentiation, informally stated]
...
thelossefficientlyforfully-connectedneuralnetworksusingbackpropagation.
...
the variables θ ,...,θ via the intermediate variable g ,...,g :
1 p 1 k
g = g (θ ,...,θ ),∀j ∈ {1,··· ,k} (7.27)
j j 1 p
J = J(g ,...,g ) (7.28)
1 k
Here we overload the meaning of g ’s: they denote both the intermediate
j
...
ways, we will call J the output variable, g ,...,g intermediate variables,
1 k
and θ ,...,θ the input variable in the chain rule.
1 p
7.3.2 One-neuron neural networks
Simplifying notations: In the rest of the section, we will consider a
generic input x and compute the gradient of h (x) w.r.t θ. For simplicity,
θ
we use o as a shorthand for h (x) (o stands for output). For simplicity, with
θ
slight abuse of notation, we use J = 1(y − o)2 to denote the loss function.
2
(Note that this overrides the definition of J as the total loss in Section 7.1.)
...
J = (y −o)2 (7.32)
2
BythechainrulewithJ astheoutputvariable,oastheintermediatevariable,
and w the input variable, we have that
i
∂J ∂J ∂o
= · (7.33)
∂w ∂o ∂w
...
variable, and w the input variable, we have that
i
∂o ∂o ∂z
= ·
∂w ∂z ∂w
...
∂w ∂o ∂z ∂w
i i
(because ∂J = (o−y) and ∂o = ReLU(cid:48)(z) and ∂z = x )
∂o ∂z ∂wi i
Here, the key is that we reduce the computation of ∂J to the computa-
∂wi
tion of three simpler more “local” objects ∂J, ∂o, and ∂z , which are much
∂o ∂z ∂wi
simpler to compute because J directly depends on o via equation (7.32), o
directly depends on a via equation (7.31), and z directly depends on w via
i
equation (7.30). Note that in a vectorized form, we can also write
∇ J = (o−y)·ReLU(cid:48)(z)·x
w
Similarly, we compute the gradient w.r.t b by
∂J ∂J ∂o ∂z
= · · = (o−y)·ReLU(cid:48)(z)
∂b ∂o ∂z ∂b
(because ∂J = (o−y) and ∂o = ReLU(cid:48)(z) and ∂z = 1)
∂o ∂z ∂b
7.3.3 Two-layer neural networks: a low-level unpacked
computation
...
∀j ∈ [1,...,m], z = w[1](cid:62) x+b[1] where w[1] ∈ Rd,b[1]
j j j j j
a = ReLU(z ),
j j
a = [a ,...,a ](cid:62) ∈ Rm
1 m
o = w[2](cid:62) a+b[2] where w[2] ∈ Rm,b[2] ∈ R
1
J = (y −o)2 (7.34)
2

95
We will use (w[2]) to denote the (cid:96)-th coordinate of w[2], and (w[1]) to denote
(cid:96) j (cid:96)
the (cid:96)-coordinate of w[1]. (We will avoid using these cumbersome notations
j
...
= (o−y)a
(cid:96)
It’s more challenging to compute ∂J . Towards computing it, we first
∂(w[1])
j (cid:96)
invoke the chain rule with J as the output variable, z as the intermediate
j
variable, and (w[1]) as the input variable.
j (cid:96)
∂J ∂J ∂z
j
= ·
∂(w j [1]) (cid:96) ∂z j ∂(w j [1]) (cid:96)
∂J
= ·x (becaues ∂zj = x .)
∂z (cid:96) ∂(w[1]) (cid:96)
j j (cid:96)
Thus, it suffices to compute the ∂J . We invoke the chain rule with J as the
∂zj
output variable, a as the intermediate variable, and z as the input variable,
j j
∂J ∂J ∂a
j
=
...
∂a
j
Now it suffices to compute ∂J , and we invoke the chain rule with J as the
∂aj
output variable, o as the intermediate variable, and a as the input variable,
j
∂J ∂J ∂o
=
∂a ∂o ∂a
...
∂J
= (o−y)·(w[2]) ReLU(cid:48)(z )x
∂(w[1]) j j (cid:96)
j (cid:96)

96
Next we gauge the runtime of computing these partial derivatives. Let p
denotesthetotalnumberofparametersinthenetwork. Wenotethatp ≥ md
where m is the number of hidden units and d is the input dimension. For
every j and (cid:96), to compute ∂J , apparently we need to compute at least
∂(w[1])
j (cid:96)
...
is inefficient.
However, the key of the backpropagation is that for different choices of (cid:96),
the formulas above for computing ∂J share many terms, such as, (o−y),
∂(w[1])
j (cid:96)
(w[2]) andReLU(cid:48)(z ). Thissuggeststhatwecanre-organizethecomputation
j j
to leverage the shared computation.
It turns out the crucial shared quantities in these formulas are ∂J,
∂o
∂J ,..., ∂J . We now write the following formulas to compute the gradi-
∂z1 ∂zm
ents efficiently in Algorithm 3.
Algorithm 3 Backpropagation for two-layer neural networks
1: Compute the values of z ,...,z , a ,...,a and o as in the definition of
1 m 1 m
neural network (equation (7.34)).
2: Compute ∂J = (o−y).
∂o
3: Compute ∂J for j = 1,...,m by
∂zj
∂J ∂J ∂o ∂a ∂J
= j = ·(w[2]) ·ReLU(cid:48)(z ) (7.35)
j j
∂z ∂o ∂a ∂z ∂o
j j j
4: Compute ∂J , ∂J , ∂J , and ∂J by
∂(w
j
[1])
(cid:96)
...
j
= · = ·x
∂(w j [1]) (cid:96) ∂z j ∂(w j [1]) (cid:96) ∂z j (cid:96)
∂J ∂J ∂z ∂J
j
= · =
∂b[1] ∂z
...
matrix variable, then ∂A denotes the collection of the partial derivatives with
∂B
the same shape as B.6 In other words, if B is a matrix of dimension m×d,
then ∂A is a matrix in Rm×d with ∂A as the ijth-entry. Let v (cid:12)w denote
∂B ∂Bij
...
2: Compute δ[2] = (o−y) ∈ R
3: Compute δ[1] = (o−y)·W[2](cid:62) (cid:12)ReLU(cid:48)(z) ∈ Rm×1
4: Compute
∂J
= δ[2]a(cid:62) ∈ R1×m
∂W[2]
...
= δ[1] ∈ Rm
∂b[1]
5We will avoid using the notation ∂A for A that is not a real-valued variable.
∂B
6Ifyouarefamiliarwiththenotionoftotalderivatives,wenotethatthedimensionality
...
z = Wu+b, where W ∈ Rm×d
J = J(z) (7.37)
Then we can compute ∂J and ∂J by:
∂u ∂W
∂J ∂J
= W(cid:62) (7.38)
∂u ∂z
...
= (7.40)
∂b ∂z
We can verify the dimensionality is indeed compatible because ∂J ∈ Rm,
∂z
W(cid:62) ∈ Rd×m, ∂J ∈ Rd, ∂J ∈ Rm×d, u(cid:62) ∈ R1×d.
∂u ∂W
...
Then, we have that
∂J ∂J
= (cid:12)σ(cid:48)(z) (7.41)
∂z ∂a
where σ(cid:48)(·) is the element-wise derivative of the activation function σ, and
(cid:12) is element-wise product of two vectors of the same dimensionality.
...
∂J ∂J
= (cid:12)ReLU(cid:48)(z) (byinvokingequation(7.41)withsetting)
∂z ∂a J ←J,a←a,z←a,σ←ReLU.
= (o−y)W[2](cid:62) (cid:12)ReLU(cid:48)(z) ( byinvokingequation(7.38)withsetting )
J ←J,z←o,W ←W[2],u←a,b←b[2]

99
Therefore, δ[1] = ∂J, and we verify the correctness of Line 3 in Algorithm 4.
∂z
Similarly, let’s verify the third equation in Line 4,
∂J ∂J
= ·x(cid:62) ( byinvokingequation(7.39)withsetting )
∂W[1] ∂z J ←J,z←z,W ←W[1],u←x,b←b[1]
= δ[1]x(cid:62) (because we have proved δ[1] = ∂J)
∂z
...
J = (a[r] −y)2
2
Here we define both a[r] and z[r] as h (x) for notational simplicity.
θ
Define
∂J
δ[k] = (7.42)
...
computes ∂J from δ[k] as described in Algorithm 5.
∂W[k]
...
J. (cid:46) This is often called the “forward pass”
2: Compute δ[r] = ∂J = (z[r] −o).
∂z[r]
3: for k = r−1 to 1 do
4: Compute
∂J (cid:16) (cid:17)
δ[k] = = W[k+1](cid:62) δ[k+1] (cid:12)ReLU(cid:48)(z[k])
∂z[k]
5: Compute
∂J
=
...
 
| | |
Z[1] =  z[1](1) z[1](2) z[1](3)  = W[1]X +b[1] (7.44)
| | |
...
Using Claim 8.1.1 with A = ξ and B = h(cid:63)(x)−h ˆ (x), we have
S
...
bution. I.e., P(Z = 1) = φ, and P(Z = 0) = 1−φ. Let φ
ˆ
= (1/n)
(cid:80)n
Z
...
and hence εˆ(h ˆ ) ≤ εˆ(h) for all h, and in particular εˆ(h ˆ ) ≤ εˆ(h∗). The third
...
122
ε(h ˆ ) ≤ ε(h∗)+2γ, with to hold with probability at least 1−δ, it suffices that
(cid:16) (cid:17) (cid:16) (cid:17)
...
(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)
(cid:0) (cid:0) (cid:1) (cid:1)(cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:1) (cid:1)(cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1)
...
(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1) (cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)
(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1) (cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)(cid:0)(cid:1)
(cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:1)(cid:0) (cid:0) (cid:1) (cid:1) (cid:0) (cid:0) (cid:1) (cid:1)(cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1) (cid:0) (cid:0) (cid:0) (cid:1) (cid:1) (cid:1)
...
puts w(cid:62)φ (x), where w ∈ Rm is a parameter to be learned, and θ ˆ is exactly
θˆ
...
1 T 1 2 1 T 1 T−1
Now the support of each of the conditional probability p(x | x ,··· ,x ) is
t 1 t−1
V.
We will model the conditional probability p(x | x ,··· ,x ) with some
t 1 t−1
parameterized form. To this end, we first turn the discrete words into word
embeddings.
Let e ∈ Rd be the embedding of the word i ∈ {1,2,··· ,V}. We call
i
[e ,··· ,e ] ∈ Rd×V the embedding matrix. The most commonly used model
1 V
...
pipeline is visualized in Figure 14.1. Given a document (x ,··· ,x ), we first
1 T
compute the corresponding word embeddings (e ,··· ,e ). Then, the word
x1 xT
...
172
vectors and outputs a sequence of vectors (c ,··· ,c ). In particular, here
2 T+1
we use the autoregressive version of the transformers which makes sure that
c only depends on x ,··· ,x .7 The c ’s are often called the representations
t 1 t−1 t
or the contextualized embeddings, and we write c = φ (x ,...,x ) where
t θ 1 t−1
φ denotes the mapping from the input to the representations.
θ
Figure 14.1: The input-output interface of a transformer.
To learn the parameters θ in the transformer, we use c to predict
t
the conditional probability p(x | x ,··· ,x ).8 We parameterize p(x |
t 1 t−1 t
x ,··· ,x ) by
1 t−1
 
...
t 1 t−1
p(x = 2 | x ··· ,x )
 t . 1 t−1  = softmax(W c ) ∈ RV (14.6)
 .  t t
.
 
p(x = V | x ··· ,x )
t 1 t−1
= softmax(W φ (x ,··· ,x )), (14.7)
t θ 1 t−1
where W ∈ RV×d is a weight matrix that maps the contextualized embedding
c to the logits. In other words, W is an additional linear layer for the
t t
prediction of the conditional probability. Recall that softmax(·) : RV (cid:55)→ RV
...
the losses are also different.
8Here t≥2and we omitthe lossfor predicting p(x )for simplicity (whichalso doesn’t
1
affect the performance much). To formally model p(x ), an option is to prepend a special
1
tokenx =⊥tothesequence,andthenaskthelanguagemodeltopredictp(x |x =⊥).
0 1 0

173
We train all the parameters θ in the transformers as well as the
parameters W = (W ,...,W ) by the cross entropy loss. Let p =
1 T t
softmax(W φ (x ,··· ,x )) ∈ RV be the predicted conditional probability
t θ 1 t−1
at position t. Let W be the concatenation of W ,··· ,W . The loss function
1 T
is often called language modeling loss and defined as
T
(cid:88)
...
t,xt
t=2
where p denotes the j-th entry of the probability vector p .
t,j t
...
0 1 2 T
···
s(n) − a 0 ( → n) s(n) − a( 1→ n) s(n) − a( 2→ n) ··· a − ( T n →− ) 1 s(n)
0 1 2 T
