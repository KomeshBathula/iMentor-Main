...
const GamificationProfileSchema = new mongoose.Schema({
    ...
    // Skill tree progress: topic -> { levels: { [levelId]: { stars, score, attempts, ... } } }
    skillTreeProgress: {
        type: Object,
        default: {}
    },
    ...
    // Deleted game history (topic + completed levels count + full game snapshot)
    deletedGames: [{
        topic: { type: String, required: true },
        completedLevels: { type: Number, default: 0 },
        totalLevels: { type: Number, default: 0 },
        savedGameData: {
            assessmentResult: { type: mongoose.Schema.Types.Mixed },
...
// Method to calculate XP level from totalXP
GamificationProfileSchema.methods.calculateXPLevel = function () {
    // XP level formula (simple): Level = floor(sqrt(totalXP / 50)) + 1
    return Math.floor(Math.sqrt((this.totalXP || 0) / 50)) + 1;
};

// Method to get XP needed for next XP level
GamificationProfileSchema.methods.getXPForNextLevel = function () {
    const current = this.xpLevel || 1;
    const next = current + 1;
    const needed = Math.pow(next - 1, 2) * 50;
    return needed - (this.totalXP || 0);
};
