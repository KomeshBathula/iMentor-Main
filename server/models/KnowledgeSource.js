...
const KnowledgeSourceSchema = new mongoose.Schema({
    ...
    embedding: {
        type: [Number], // For vector search (FAISS/MongoDB Vector)
        select: false // Avoid pulling massive arrays on standard queries
    },
    version: {
        type: Number,
        default: 1
    ...
    contentHash: {
        type: String, // MD5 or SHA256 of textContent for incremental updates
        index: true
    },
    credibilityScore: {
        type: Number,
        default: 80 // Local sources are highly trusted
    },
    originSource: {
        type: String // Where it originally came from (internal wiki, user upload, etc)
    },
    analysis: {
        type: AnalysisSchema,
        default: () => ({}),
